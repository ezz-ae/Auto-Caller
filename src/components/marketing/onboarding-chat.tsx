    ...
    <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/70 backdrop-blur-sm shadow-2xl shadow-black/50 overflow-hidden flex flex-col h-[460px]">
      ...
      <div className="px-5 py-4 border-b border-zinc-800/60 flex items-center gap-3 bg-zinc-900/90 shrink-0">
        <div className="relative shrink-0">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-cyan-500 to-cyan-700 flex items-center justify-center shadow-md shadow-cyan-500/20">
            <Bot className="w-4 h-4 text-white" />
          </div>
          <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-cyan-400 border-2 border-zinc-900" />
        </div>
        <div>
          ...
          >
            <div
              className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                msg.role === 'user'
                  ? 'bg-cyan-500/20 text-cyan-50 border border-cyan-500/25 rounded-tr-sm'
      ...
      <div className="px-4 pb-4 pt-3 border-t border-zinc-800/50 bg-zinc-900/60 shrink-0">
        {phase === 'done' ? (
          <div className="flex flex-col gap-2">
            <Button
              asChild
              className="w-full h-11 rounded-xl bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 font-semibold shadow-lg shadow-cyan-500/20 text-white"
            >
              <Link href="/login">
            ...
            <button
              type="button"
              className="text-[11px] text-zinc-500 hover:text-cyan-400 transition text-center py-1 flex items-center justify-center gap-1.5"
            ...
            {!isTyping &&
              chips.map(chip => (
                <button
                  ...
                  className="px-3 py-1.5 rounded-full border border-zinc-700/80 bg-zinc-800/50 text-xs font-medium text-zinc-300 hover:border-cyan-500/60 hover:text-cyan-300 hover:bg-cyan-500/8 transition-all duration-150"
                >
                  {chip}
                </button>
              ))}
