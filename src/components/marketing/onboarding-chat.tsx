    ...
    <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/70 backdrop-blur-sm shadow-2xl shadow-black/50 overflow-hidden flex flex-col h-[460px]">
      ...
      <div className="px-5 py-4 border-b border-zinc-800/60 flex items-center gap-3 bg-zinc-900/90 shrink-0">
        <div className="relative shrink-0">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center shadow-md shadow-emerald-500/20">
          ...
          >
            <div
              className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                msg.role === 'user'
                  ? 'bg-emerald-500/20 text-emerald-50 border border-emerald-500/25 rounded-tr-sm'
      ...
      <div className="px-4 pb-4 pt-3 border-t border-zinc-800/50 bg-zinc-900/60 shrink-0">
        {phase === 'done' ? (
          <div className="flex flex-col gap-2">
            <Button
              asChild
              className="w-full h-11 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 font-semibold shadow-lg shadow-emerald-500/20 text-white"
            >
              <Link href="/login">
            ...
            <button
              type="button"
              className="text-[11px] text-zinc-500 hover:text-emerald-400 transition text-center py-1 flex items-center justify-center gap-1.5"
            ...
            {!isTyping &&
              chips.map(chip => (
                <button
                  ...
                  className="px-3 py-1.5 rounded-full border border-zinc-700/80 bg-zinc-800/50 text-xs font-medium text-zinc-300 hover:border-emerald-500/60 hover:text-emerald-300 hover:bg-emerald-500/8 transition-all duration-150"
                >
                  {chip}
                </button>
              ))}
