...
export function MarketingHeader() {
  return (
    <header className="sticky top-0 z-30 border-b border-blue-300/20 bg-zinc-950/75 backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between gap-4">
        <BrandLogo showTagline />

        <nav className="hidden lg:flex items-center gap-6 text-sm text-zinc-300">
          {navItems.map(item => (
            <Link key={item.href} href={item.href} className="hover:text-blue-200 transition">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button asChild className="bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-500 hover:to-blue-700 font-semibold">
...
export function MarketingFooter() {
  return (
    <footer className="border-t border-blue-300/15 bg-zinc-950/65">
      <div className="mx-auto max-w-7xl px-4 py-10 grid gap-8 md:grid-cols-4">
        ...
        <div>
          ...
          <div className="space-y-2 text-sm text-zinc-400">
            <Link href="/features" className="block hover:text-blue-200 transition">Features</Link>
            <Link href="/how-it-works" className="block hover:text-blue-200 transition">How It Works</Link>
            <Link href="/login" className="block hover:text-blue-200 transition">Start now</Link>
          </div>
        </div>
        <div>
          <p className="text-sm font-semibold text-white mb-3">Resources</p>
          <div className="space-y-2 text-sm text-zinc-400">
            <Link href="/docs" className="block hover:text-blue-200 transition">Docs</Link>
            <Link href="/faq" className="block hover:text-blue-200 transition">FAQ</Link>
            <Link href="/login" className="block hover:text-blue-200 transition">Workspace Login</Link>
          </div>
        </div>
        <div>
