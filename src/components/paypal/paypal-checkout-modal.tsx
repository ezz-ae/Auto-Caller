        ...
        <div className="space-y-1.5">
          ...
          {result.assignedPhoneNumber ? (
            <p className="text-sm text-zinc-400">
              Your number <span className="text-emerald-400 font-mono">{result.assignedPhoneNumber}</span> is now active.
            </p>
          ) : (
            <p className="text-sm text-zinc-400">
              <span className="text-emerald-400 font-bold">{result.credits?.toLocaleString()}</span> credits added to your balance.
            </p>
          )}
        </div>
        <Button
          onClick={onClose}
          className="w-full max-w-xs h-11 rounded-xl bg-emerald-500 hover:bg-emerald-600 font-semibold"
        >
          Done
        </Button>
      </div>
