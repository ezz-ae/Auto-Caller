  ...
  if (state === 'success' && result) {
    return (
      <div className="flex flex-col items-center gap-5 py-4 text-center">
        <div className="w-16 h-16 rounded-full bg-orange-400/15 flex items-center justify-center">
        ...
        <div className="space-y-1.5">
          ...
          {result.assignedPhoneNumber ? (
            <p className="text-sm text-zinc-400">
              Your number <span className="text-orange-300 font-mono">{result.assignedPhoneNumber}</span> is now active.
            </p>
          ) : (
            <p className="text-sm text-zinc-400">
              <span className="text-orange-300 font-bold">{result.credits?.toLocaleString()}</span> credits added to your balance.
            </p>
          )}
        </div>
        <Button
          onClick={onClose}
          className="w-full max-w-xs h-11 rounded-xl bg-orange-400 hover:bg-orange-500 font-semibold"
        >
          Done
        </Button>
      </div>
