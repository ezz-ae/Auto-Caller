      ...
      <Card className="bg-zinc-900 border-zinc-800 shadow-xl overflow-hidden">
        <CardHeader className="pb-8 bg-zinc-950/50 border-b border-zinc-800/50">
          <div className="flex items-center justify-between">
            ...
            {activeAgentId && (
              <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 px-4 py-1.5 rounded-full text-sm font-bold">
                  ...
                  >
                    <div className={`max-w-[80%] rounded-2xl p-5 shadow-sm ${
                      msg.role === 'user' 
                        ...
                        : 'bg-emerald-500/10 border border-emerald-500/20 text-zinc-200 rounded-tl-none'
                    }`}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {msg.role === 'assistant' && (
                          <Button
                            ...
                            size="sm"
                            variant="ghost"
                            className="h-6 px-2 text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10 text-[10px] uppercase font-bold"
                      ...
                      {msg.checklist && msg.checklist.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-emerald-500/10 space-y-2">
                          <p className="text-[10px] uppercase font-bold text-emerald-400 tracking-widest">Recommended Steps</p>
                      ...
                      {msg.action && msg.action !== 'none' && getActionLabel(msg.action) && (
                        <div className="mt-4 flex items-center justify-between gap-4 p-3 rounded-xl bg-zinc-950/40 border border-zinc-800">
                          <div className="space-y-0.5">
                             ...
                             <p className="text-xs font-semibold text-emerald-400">{getActionLabel(msg.action)}</p>
                          </div>
            ...
            <div className="p-6 bg-zinc-900 border-t border-zinc-800">
              <div className="flex gap-3 max-w-4xl mx-auto">
                <Input
                  ...
                  className="bg-zinc-800/50 border-zinc-700 h-12 rounded-xl text-zinc-100 placeholder:text-zinc-600 px-6 focus-visible:ring-emerald-500/50"
                  disabled={!activeAgentId}
                />
                <Button 
                  onClick={askAgent} 
                  disabled={agentLoading || !agentInput.trim() || !activeAgentId}
                  className="h-12 w-12 rounded-xl bg-emerald-500 hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 flex items-center justify-center shrink-0"
                >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${isActive ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-900 text-zinc-600 group-hover:text-zinc-400'}`}>
