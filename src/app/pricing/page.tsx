      ...
      <main className="mx-auto max-w-7xl px-4 py-12 md:py-16 space-y-12">
        ...
        <section className="grid gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
          {sortedProducts.map(product => {
            ...
            return (
              <Card key={product.id} className={`relative bg-gradient-to-br from-zinc-900/90 via-zinc-900/70 to-zinc-950/80 border-sky-400/15 ${isPopular ? 'border-sky-400/60 shadow-lg shadow-sky-500/10' : ''}`}>
                {isPopular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <Badge className="bg-sky-500 text-white">Best Value</Badge>
                  </div>
                )}
                <CardHeader className="text-center pb-4">
                  <div className={`w-14 h-14 rounded-xl mx-auto mb-3 flex items-center justify-center ${
                    isPopular ? 'bg-sky-500/20 text-sky-300' : 'bg-zinc-800 text-zinc-400'
                  }`}>
        ...
        <section className="rounded-xl border border-sky-400/20 bg-gradient-to-r from-sky-500/10 via-blue-500/5 to-transparent p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
